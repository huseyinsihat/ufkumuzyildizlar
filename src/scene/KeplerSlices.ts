...
import { educationalOrbitRadius, TRUE_SCALE_AU_UNITS } from '../astronomy/visualScale'
import {
  equalTimeSector,
  keplerSpeedFactor,
  orbitPointFlat,
...
export class KeplerSlices {
  ...
  constructor() {
    ...
    this.peri = new Mesh(new SphereGeometry(0.55, 12, 10), new MeshBasicMaterial({ color: '#fb7185' }))
    this.arrow = new Mesh(new ConeGeometry(0.42, 1.6, 8), new MeshBasicMaterial({ color: '#fde68a' }))
    this.near.raycast = () => {}
    this.far.raycast = () => {}
    this.peri.raycast = () => {}
    this.arrow.raycast = () => {}
    this.group.add(this.near, this.far, this.peri, this.arrow)
  }
  ...
  rebuild(mode: ScaleMode): void {
    ...
    this.near.geometry = fanGeometry(equalTimeSector(a, e, 0, HALF))
    this.far.geometry = fanGeometry(equalTimeSector(a, e, Math.PI, HALF))
  ...
  private elements(): { a: number; e: number } {
    ...
    const exaggerate = this.mode !== 'trueScale'
    const e = visualEccentricity(body.eccentricity, exaggerate)
    const a =
  ...
  dispose(): void {
    ...
    const mats = [this.near.material, this.far.material, this.peri.material, this.arrow.material]
